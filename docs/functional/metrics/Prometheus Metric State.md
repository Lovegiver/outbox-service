...
mis à jour dans la même transaction. Chaque flux possède sa propre frontière
transactionnelle : l'échec d'un flux annule uniquement son état et son
checkpoint, puis le worker poursuit les autres flux du cycle.

## Noms et labels

Les caractères d'un `metric_code` hors `[a-zA-Z0-9_:]` sont remplacés par
`_`. Le préfixe de métrique `ob1_` est ensuite ajouté une seule fois ; un code
déjà préfixé reste accepté sans recevoir un second préfixe. Il garantit aussi
...
`labels_hash`. La réservation du préfixe `ob1_` concerne les noms de labels
métier : tout label métier dont le nom commence par `ob1_` est rejeté.
